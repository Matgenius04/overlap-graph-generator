...
class Range {
  public start: number;
  public end: number;
  protected inputStart: HTMLInputElement;
  protected inputEnd: HTMLInputElement;
  ...
  constructor(start: number, end: number, parent: HTMLDivElement, label?: string, color?: string) {
    ...
    this.inputStart.step = String(0.1 ** precision(this.end))
    this.inputEnd.step = String(0.1 ** precision(this.end))
  }
...
class Scale extends Range {
  ...
  public generateTickMarkers(mode: 'amount' | 'interval', amount?: number, interval?: number, startDelta?: number) {
    if (mode == "amount" && (isFinite(amount as number) ? amount as number > 1 : false)) {
      return (Array<number>(amount as number)).fill(0).map((_v, i) => {
        return Math.round(((i * this.delta() / ((amount as number) - 1)) + this.start + Number.EPSILON) * 100000) / 100000
      })
    } else if (mode == "interval" && (isFinite(interval as number) ? interval : false)) {
      let j = 0;
      const ticks = [];
      let i = this.start + (startDelta || 0);
      while (i <= this.end && j < 10000) {
        ticks.push(Math.round(((i + Number.EPSILON) * 10000)) / 10000)
        i += interval as number
        j++
      }
      if (j == 10000) throw "Infinite Loop"
      return ticks
    } else {
  ...
  public update(start: number | null, end: number | null) {
    ...
    console.log(precision(this.start).toString())
    this.inputStart.step = String(0.1 ** precision(this.end))
    this.inputEnd.step = String(0.1 ** precision(this.end))
  }
  public storeValues() {
...
// taken from: https://stackoverflow.com/questions/9553354/how-do-i-get-the-decimal-places-of-a-floating-point-number-in-javascript
